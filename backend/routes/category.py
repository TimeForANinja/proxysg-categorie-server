...
def add_category_bp(app: APIFlask):
    ...
    def create_category(category_data: CategoryInput) -> CategoryOutput:
        ...
        db = get_db()
        category = db.categories.create_category(user.get_branch(), category_data.name)
    ...
    def update_category(category_id: str, category_data: CategoryInput) -> OutCanError[CategoryOutput]:
        ...
        db = get_db()
        category, error = db.categories.update_category(user.get_branch(), category_id, category_data.name)
    ...
    @category_bp.delete('/api/branch/@me/category/<category_id>/url/<url>')
    ...
    @category_bp.output(generic_output_schema)
    @category_bp.auth_required(auth, roles=[AuthRoles.RW])
    def delete_category_url(category_id: str, url: str) -> GenericOutput:
        user: AuthUser = cast(AuthUser, auth.current_user)

        db = get_db()
        error = db.mappings.delete_url_category(user.get_branch(), category_id, url)
